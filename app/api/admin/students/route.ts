...
export async function POST(req: Request) {
  ...
  const { name, email, password, studentId, joinDate, feeStatus, gender, phone } = parsed.data;
  ...
  const { data: user, error: userErr } = await supabase
    .from('users')
    .insert({ email: email.toLowerCase(), name, role: 'student', phone: phone || null })
    .select('id')
    .single();
...
export async function GET() {
  ...
  const { data, error } = await supabase
    .from("students")
    .select("id, user_id, student_id, gender, user:users(id, email, name)")
